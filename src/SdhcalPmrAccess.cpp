...
uint32_t PMRUnpacker::getID(unsigned char* cb,uint32_t idx)
{
  return cb[idx+DU_ID_SHIFT];
}


uint32_t PMRUnpacker::getGTC(unsigned char* cb,uint32_t idx)
{
...
uint32_t PMRUnpacker::getBCID(unsigned char* cb,uint32_t idx)
{
  return (cb[idx+DU_BCID_SHIFT]<<16)+(cb[idx+DU_BCID_SHIFT+1]<<8)+cb[idx+DU_BCID_SHIFT+2];
}
uint32_t PMRUnpacker::getLastTriggerBCID(unsigned char* cb,uint32_t idx)
{
...
uint32_t PMRUnpacker::getFrameAsicHeader(unsigned char* framePtr)
{
  return (framePtr[DU_FRAME_ASIC_HEADER_SHIFT]);
}
void PMRUnpacker::setFrameAsicHeader(unsigned char* framePtr,uint8_t v)
{
  framePtr[DU_FRAME_ASIC_HEADER_SHIFT]=v;
}
...
uint32_t PMRUnpacker::getFramePtr(std::vector<unsigned char*> &vFrame,std::vector<unsigned char*> &vLines,uint32_t max_size,unsigned char* cb,uint32_t idx) throw (std::string)
{
